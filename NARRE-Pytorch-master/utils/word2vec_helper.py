...
def review2wid(review: str, word_vec: Word2VecKeyedVectors) -> List[int]:
    ...
    wids = []
    pad_index = word_vec.key_to_index[PAD_WORD]
...
def get_word_vec(path='data/GoogleNews-vectors-negative300.bin'):
    ...
    assert PAD_WORD_ID == word_vec.key_to_index[PAD_WORD], \
        f"PAD_WORD_ID should be {word_vec.key_to_index[PAD_WORD]} but not {PAD_WORD_ID}."
...
if __name__ == "__main__":
    df = pandas.read_json(ROOT_DIR.joinpath("reviews.json"), lines=True)
